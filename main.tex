...
\begin{abstract}
We formalize the \emph{Wicked} Prior as an instantiation of bounded symbolic geometry under dual-horizon constraints. Oz is modeled as a bounded symbolic manifold $\Mani$ equipped with an outer projection metric $g_{\mathrm{out}}$ (social legibility) and an inner emergence metric $g_{\mathrm{in}}$ (truth-seeking). Part I exhibits resolution collapse $\epsres\!\to\!0$ that concentrates curvature and produces atlas fracture at the narrative climax. Part II repairs the manifold via a Ricci-type grace flow with adaptive cadence $\phi(\tau)$, enabling reconciliation while preserving identity. We provide (i) a computable curvature estimator on screenplay embeddings predicting a dominant peak at \emph{Defying Gravity}, (ii) a Stackelberg parentage paradox showing the Wizard misclassifies his own generated corrective as adversarial, (iii) categorical conditions for reconciliation as equivalence of identity-trajectory categories, and (iv) AI corollaries: resolution floors as spectral minima, grace scheduling as multi-loss flow, and connection-preservation monitoring. Each claim is paired with a validation protocol and falsification criterion.
\end{abstract}
...
\subsection{Prediction (Part I)}
\begin{tcolorbox}[colback=predictionorange,colframe=ozgreen,title={\textbf{Prediction P1: Defying Gravity Curvature Peak}}]
\textbf{P1.} The dominant curvature peak aligns with the \emph{Defying Gravity} climax.  

\textbf{Controls.} Resample scenes to uniform token-length arc and repeat across multiple embedding models.  

\textbf{Falsification.} If the maximum $\kappa_t$ occurs far from the climax ($>2$ scenes), or if peaks drift substantially across embedding families, then $\kappa_t$ is not tracking a narrative invariant.
\end{tcolorbox}
...
Let $t^\star$ denote the window(s) containing Chapter~15 (``The Discovery
of Oz, the Terrible''), where the curtain falls and the Wizard is revealed
as an ordinary man.\cite{Baum1900}
Under the dual-horizon hypothesis, we predict a measurable discontinuity in
semantic entropy around $t^\star$:
\[
  H_{t^\star - 1} \approx H_{t^\star + 1}
  \quad\text{but}\quad
  \Delta H_{t^\star}
  \;=\;
  \bigl|H_{t^\star} - H_{t^\star - 1}\bigr|
  \gg 0,
\]
reflecting a transient shock as the text transitions from a single-chart
description (unquestioned belief in the Wizard) to an atlas with multiple
incompatible frames (projection versus reality).

\paragraph{Interpretation.}
A statistically significant entropy spike at $t^\star$ would support the
claim that the unmasking constitutes a genuine chart transition in the
Baum manifold, not merely a local plot twist.
Failure to observe such a spike, especially under multiple robust choices
of windowing and embedding, would falsify this specific instantiation of
our geometric reading while leaving the broader formalism intact.
Either outcome is informative: V-Baum turns the public-domain text into a
reproducible testbed for the dual-horizon model rather than a purely
